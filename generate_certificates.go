...
func makeIntermediateAuthorityCertificate() {
	...
	if !fileExists(stringFragments["intermediateAuthorityCertificate"]) {
		...
		generateSignedCertificate(stringFragments["intermediateAuthorityCSR"], stringFragments["intermediateAuthorityCertificate"], stringFragments["rootAuthorityMakeCertificateConfiguration"], stringFragments["rootAuthorityPrivateKey"], stringFragments["rootAuthorityCertificate"], stringFragments["intermediateAuthorityDirectory"])
	}
}
...
func makeRootAuthorityCertificate() {
	...
	if !fileExists(stringFragments["rootAuthorityCertificate"]) {
		if !fileExists(stringFragments["rootAuthorityMakeCertificateConfiguration"]) {
			fmt.Println("No root authority configuration file detected in " + stringFragments["rootAuthorityMakeCertificateConfiguration"] + ". Copying from templates directory")
...
func initializeStringFragments() {
	...
	stringFragments["intermediateAuthorityConfiguration"] = stringFragments["intermediateAuthorityDirectory"] + "/" + stringFragments["intermediateAuthorityMakeCertificateConfigurationFilename"]
