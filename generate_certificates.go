...
func main() {
	...
	if !fileExists(rootCertificate) {
		...
		if !fileExists(rootAuthorityConfigurationFilepath) {
			...
			fileCopy(templatesDirectoryRootAuthorityConfigFilepath, rootAuthorityConfigurationFilepath)

			contentAsBytes, err := ioutil.ReadFile(rootAuthorityConfigurationFilepath)
			if err != nil {
				fmt.Println("Error when reading " + rootAuthorityConfigurationFilepath)
				fmt.Println(err)
			}
