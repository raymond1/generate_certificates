...
ocsp_responder.crt: ocsp_responder.csr ocsp_responder_csr.conf intermediate.pem intermediate.crt
	openssl ca -in ocsp_responder.csr -out ocsp_responder.crt -config intermediate_ca.conf -keyfile intermediate.pem -cert intermediate.crt -outdir intermediate_certificates -batch

ocsp_responder.csr: ocsp_responder.pem ocsp_responder_csr.conf intermediate.crt
	openssl req -key ocsp_responder.pem -in intermediate.crt -days 398 -new -config ocsp_responder_csr.conf -out ocsp_responder.csr
...
directories:
	mkdir -p root_certificates intermediate_certificates
